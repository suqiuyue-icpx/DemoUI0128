...
class userinfo(Page):
    ...
    # 输入用户姓名
    username_loc = (By.ID, testData.get_elementinfo(10))
    # 输入用户账号
    usercode_loc = (By.ID, testData.get_elementinfo(11))
    # 输入用户密码
    userpassword_loc = (By.ID, testData.get_elementinfo(12))
    # 输入用户组织
    userOrg_loc = (By.ID, testData.get_elementinfo(13))
    # 点击保存按钮
    save_button_loc = (By.XPATH, testData.get_elementinfo(14))
    ...
    def input_userOrg(self, userOrg):
        """
        输入用户组织
        :param userOrg:
        :return:
        """
        self.find_element(*self.userOrg_loc).clear()
        self.find_element(*self.userOrg_loc).send_keys(userOrg)
